            ...
            <div class="post-content">
              <div class="post-image">
                <a class="fancybox" rel="group" href="{{ $new->full_image }}">
                  <img src="{{ $new->full_image }}" alt="{{ $new->$name }}">
                </a>
              </div>
              <div class="date-box">
                <div class="day">{{ $new->created_at->format('d') }}</div>
                <div class="day">{{ $new->month }}</div>
              </div>
              <div class="post-text">
                <h3><a href="{{ route('new', [ 'lang' => $lang,'permalink' => $new->permalink]) }}" class="marg-r-80 color-black">{{$new->$name}}</a></h3>
                <p class="marg-r-80 min-h">{{ str_replace ('&nbsp;' , ' ' , strip_tags ( clean_limit($new->$description,300))) }}</p>
              </div>
              <div class="t-left">
              <a href="{{ route('new', ['lang' => $lang, 'permalink' => $new->permalink]) }}" class="btn-more">@lang('site.read_more')</a>
              </div>
            </div>
          </li>
          @endforeach
