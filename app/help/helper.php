...
function uploadImageResponsive($upload){
        $data = getimagesize($upload);
        $orginal_width = $data[0];
        $orginal_height = $data[1];
        $new_width=750;

        $new_height=($new_width/$orginal_width)*$orginal_height;
        ...
        $thumb = Image::make($upload)->resize($new_width, $new_height)->encode($upload->getClientOriginalExtension(), 75);
